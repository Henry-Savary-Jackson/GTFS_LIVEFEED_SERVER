...
from config import PROTOBUF_FEED_LOCATION
import datetime
from db_utils import agency_exists, get_number_of_stoptimes,trip_exists,stop_on_route, stop_exists, route_exists
...
def get_feed_object_from_file():
    if not os.path.exists(PROTOBUF_FEED_LOCATION):
        feed = get_empty_feed_message()
        save_feed_to_file(feed)
        return feed

    with open(PROTOBUF_FEED_LOCATION, "rb") as feed_bin:
        feed_object = gtfs_rt.FeedMessage()
        feed_object.ParseFromString(feed_bin.read())
        return feed_object


def save_feed_to_file(feed_message: gtfs_rt.FeedMessage):
    with open(PROTOBUF_FEED_LOCATION, "wb") as feed_bin:
...
def verify_trip_descriptor(trip_desc:dict):
    if "trip_id" in trip_desc:
        raise ValueError("You must use trip_id in your trip update's trip descriptor.")
    if not trip_exists(trip_desc["trip_id"]) :
        raise ValueError(f"Trip with trip_id \"{trip_desc["trip_id"]}\" doesn't exist.")
    return trip_desc["trip_id"]
...
def verify_trip_update(trip_update:dict):
    ...
    for stop_time_update in trip_update["stop_time_update"]:
        if "stop_sequence" not in stop_time_update : 
            raise ValueError(f"A stop time update needs the stop_sequence.")
        if stop_time_update["stop_sequence"] > number_stop_times:
...
def verify_entity_selector(informed_entity:dict):
    has_agency_id = "agency_id" in informed_entity
    has_route =  "route_id" in informed_entity
    has_stop_id = "stop_id" in informed_entity
    has_trip_desc = "trip" in informed_entity 
    if has_agency_id:
        if not agency_exists(informed_entity["agency_id"]):
            raise ValueError(f"No such agency with agency_id \'{informed_entity["agency_id"]}\'")
    elif has_route and has_stop_id:
        if not stop_on_route(informed_entity["stop_id"], informed_entit["route_id"]):
            raise ValueError(f"No such stop \"{informed_entity["stop_id"]}\" is on route \"{informed_entity["route_id"]}\"")
    elif has_route :
        if not route_exists(informed_entity["route_id"]):
            raise ValueError(f"Route with route_id  \"{informed_entity["route_id"]}\" doesn't exist.")
    elif has_stop_id:
        if not stop_exists(informed_entity["stop_id"]):
            raise ValueError(f"Stop with stop_id \"{informed_entity["stop_id"]}\" doesn't exist.")
    elif has_trip_desc:
