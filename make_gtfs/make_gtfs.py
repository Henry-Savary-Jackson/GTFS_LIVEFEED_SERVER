...
GTFS_VALIDATOR_PATH =os.path.normpath(f"{FILE_PATH}/gtfs-validator.jar")
...
def get_metadata(sheet_name, services, shapes):
...
def read_schedule_as_df(sheet_name):
    return pd.read_excel(EXCEL_FILENAME, sheet_name=sheet_name, skiprows=2)  

def add_schedule(sheet_name, stoptime_df:pd.DataFrame, trip_df, route_id, services, shapes, stops):
    try: 
        service_id, shape_id = get_metadata(sheet_name, services, shapes)
        df_schedule= read_schedule_as_df(sheet_name)
...
def main(logs):
    ...
    for route in directory.iter_cols():
        ...
        # get route id 
        route_id = routes_df[routes_df["route_short_name"] == route_name].iloc[0]["route_id"] 
        for sheet_cell in route[1:]:
            try:
                ...
                stop_time_df, trip_df = add_schedule(sheet_name,stop_time_df, trip_df, route_id, services, shapes, stops)
                logs.insert(tk.END,f"Finished sheet: {sheet_name}\n")
            except Exception as e:
                logs.insert(tk.END,f"{e}\n")
                print(sheet_name, e)
